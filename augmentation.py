...
import format_keypoints_anno as fka
...
if __name__ == '__main__' : 
    ...
    for iter in range(0,3) :
        for idx, (img_path, annotation) in enumerate(zip(imgs_paths, annotations)):
            ...
            if iter == 1 : 
                idx = len(imgs_paths) + idx
            save_file_name = os.path.join(output_dir, f'{fileName}_{idx:04d}.jpg')
            ...
            if len(transformed['keypoints']) == 4 : 
                ...
                tf_box = [(round(x1, 2), round(y1, 2), round(x2, 2), round(y2, 2)) for x1, y1, x2, y2 in tf_box]
                anno = fka.make_annotation(bbox=tf_box, category_id=1, id=idx, image_id=idx, keypoints=tf_keypoints)
                img_ann = fka.make_image(file_name=save_file_name, height=oh, width=ow, id=idx)
                new_annos.append(anno)
                img_annos.append(img_ann)
                cv2.imwrite(save_file_name, transformed['image'])
                test_path = os.path.join(test_dir, f'{fileName}_{idx:04d}.jpg')
    ...
    categories = fka.make_categories("license-plate")
    data = fka.make_data(anntations=new_annos, images=img_annos, categories=categories)
