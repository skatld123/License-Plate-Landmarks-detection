...
ko_font = ImageFont.truetype('/root/Virtual_Number_Plate/font/HANGIL.ttf',
                             100, encoding='unic')
# 숫자 폰트 정보
# https://fonts.google.com/noto/specimen/Noto+Sans+KR
font = ImageFont.truetype('/root/Virtual_Number_Plate/font/NotoSansKR-Medium.otf',
                          120, encoding='unic')
...
def make_virtual(input_file, output_path) :
    # new_img_path, old_img_path, count = opt.new_plate, opt.old_plate, opt.count
    filename = os.path.basename(input_file)
    split_lp = split_korean_number(filename)
    # 소요 시간 체크
    start = time.time()

    # 8자리 신형 번호판
    for i in tqdm(range(1), desc='8자리 신형 번호판'):
        front = f'{random.randint(100, 999)}'
        middle = random.choice(korean)
        back = f' {random.randint(1000, 9999)}'
        full_name = front + middle + back

        image_pil = Image.open('/root/clp_landmark_detection/virtual-plate/number_plate_old.png')
        draw = ImageDraw.Draw(image_pil)
        draw.text((85, -20), front, 'black', font)  # (x,y), 번호판 문자열, 폰트 색, 위에서 설정한 폰트
        draw.text((290, 35), middle, 'black', ko_font)
        draw.text((375, -20), back, 'black', font)

        image_pil.save('output_path' + full_name + '.png', 'PNG')
        
def split_korean_number(string):
    numbers = '0123456789'
    korean = '가나다라마거너더러머버서어저고노도로모보소오조구누두루무부수우주하허호바사아자배'
    num = ''
    kor = ''
    result = []
    for char in string:
